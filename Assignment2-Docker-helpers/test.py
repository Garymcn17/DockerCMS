...
while selection != 13:
    ...
    if selection == 13:
        ...
        os.system("curl -s -X DELETE -H 'Accept: application/json' http://localhost:8080/containers/%s" % id)
    elif selection == 7:
        #delete all images
        ...
        os.system(
            "curl -X POST -H 'Content-Type: application/json' http://localhost:8080/containers -d '{\"image\": \"%s\"}'" % id)
    elif selection == 10:
        #Create a new image
        os.system("curl -H 'Accept: application/json' -F file=@/Dockerfile http://localhost:8080/images")
    elif selection == 11:
        #update container
        ...
        while True:
            ...
            if change == '1':
                os.system(
                    "curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/%s -d '{\"state\": \"running\"}'" % id)
                break
            elif change == '2':
                os.system(
                    "curl -X PATCH -H 'Content-Type: application/json' http://localhost:8080/containers/%s -d '{\"state\": \"stopped\"}'" % id)
                break
            else:
        ...
        os.system(
            "curl -s -X PATCH -H 'Content-Type: application/json' http://localhost:8080/images/%s -d '{\"tag\": \"test:1.0\"}'" % id)
